...
export function TaskCard({ task, onDelete, onClick, onAccept, onReject }: TaskCardProps) {
  const dateDisplay = format(task.startDate, "d 'de' MMMM", { locale: es }) === format(task.endDate, "d 'de' MMMM", { locale: es })
    ? format(task.startDate, "d 'de' MMMM, yyyy", { locale: es })
    : `${format(task.startDate, "d 'de' MMM", { locale: es })} - ${format(task.endDate, "d 'de' MMM, yyyy", { locale: es })}`

  const studentDisplay = task.students.length > 1 
    ? `${task.students.length} estudiantes`
    : task.students[0].name

  return (
    <Card 
      className="bg-white shadow-md hover:shadow-lg transition-shadow border-2 border-gray-300"
    >
